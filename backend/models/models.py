...
class Evaluaciones(db.Model):
    ...
    def __init__(self, id, id_consecutivo, nombre, fecha_insercion=None, esta_eliminada=False):
        self.id = id
    ...
    def __repr__(self):
        return f"{self.id} - {self.id_consecutivo} - {self.nombre} - {self.fecha_insercion} - {self.esta_eliminada}"
